# JS异步编程模型
再理解js异步编程时， 我们先再心中想一下为什么js语言会引入异步任务？异步到底解决了哪些问题？理解了这些之后，我们才能更好地运行异步编程思想去书写我们的业务代码逻辑。。。下面写一下个人对异步模型的理解
...
而执行栈只是作为任务的消费者而已，真正生产异步任务的生产者是：浏览器那些DOM API，网络线程，计时器线程； 
node环境下的事件等。。。
...
其实promise的核心就是then方法, 源码中也用到发布订阅模式思想, 通过then链的 链式回调将上一步结果透传给下一步使用,
解决了回调地狱的问题
